...
async function handleCommand(command, params) {
  switch (command) {
    ...
    case "create_triangle":
      return await createTriangle(params);
    case "create_star":
...
const setCharacters = async (node, characters, options) => {
  ...
  try {
    ...
    console.warn(
      `Failed to load \"${node.fontName["family"]} ${node.fontName["style"]}\" font and replaced with fallback \"${fallbackFont.family} ${fallbackFont.style}\"`, err
    );
...
async function createCircle(params) {
  const {
    ...
    size = 100,
    name = "Circle",
    color = { r: 0.8, g: 0.3, b: 0.3 }, // A reddish color
  } = params || {};
...
async function createTriangle(params) {
  const {
    x = 0,
    y = 0,
    size = 100,
    name = "Triangle",
    color = { r: 0.3, g: 0.3, b: 0.8 }, // A bluish color
  } = params || {};

  const triangle = figma.createPolygon();
  triangle.name = name;
  triangle.resize(size, size);
  triangle.x = x;
  triangle.y = y;
  triangle.pointCount = 3;
  triangle.fills = [{ type: "SOLID", color: color }];

  figma.currentPage.appendChild(triangle);
...
async function createButton(params) {
  ...
  buttonFrame.x = x;
  buttonFrame.y = y;
  buttonFrame.fills = [{ type: "SOLID", color: color }];
  ...
  // Select and zoom to the new button
